...
nGIST is currently equipped with one routine to spatially bin the data. This routine is included in the source and exploits the adaptive Voronoi tesselation routine of Cappellari & Copin (2003). It can readily be used by setting the configuration parameter `SPATIAL_BINNING:METHOD` to `'voronoi'`. Coming soon: Powerbin implementation.
